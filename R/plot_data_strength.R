...
grafico = function(x, nombres, variable = "Variable"){
...
# x <- HABD; variable = "HABD"; nombres = strength[, 3]
nombres <- strength %>% pull(3)
grafico(HABD, nombres = nombres, variable = "HABD")
grafico(HADD, nombres = nombres, variable = "HADD")
grafico(HER, nombres = nombres, variable = "HER")
grafico(HIR, nombres = nombres, variable = "HIR")
grafico(HF, nombres = nombres, variable = "HF")
grafico(KE, nombres = nombres, variable = "KE")
grafico(KF, nombres = nombres, variable = "KF")
grafico(HE, nombres = nombres, variable = "HE")
...
#' @TODO: handle the subj_id duplicates 
...
subj_idx_levels <- sort(unique(strength$subj_idx))
# subj_idx_labels <- paste0("ID ", subj_idx_levels)
subj_idx_labels <- paste0(subj_idx_levels)
...
plt_df_long <- 
  ...
  mutate(subj_idx_fct = factor(subj_idx, levels = subj_idx_levels, labels = subj_idx_labels)) %>%
...
for (i in 1 : length(var_name_unq)) { #i <- 1
  ...
  plt <- 
    ggplot(plt_df_long_i, aes(x = session_part_fct, y = value, group = subj_id)) + 
    geom_line(alpha = 0.5) + 
    geom_point(color = "grey30") + 
    # https://ggrepel.slowkow.com/articles/examples.html
    geom_text_repel(
      data = plt_df_long_i %>% filter(session_part == "Start"),
      aes(label = subj_idx_fct),
